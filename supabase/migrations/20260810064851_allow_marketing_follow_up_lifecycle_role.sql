-- Production profiles historically use both "Marketing" and "Marketing Team".
-- Keep both canonical values authorized without changing stored profile data.
...
begin
  ...
  if v_role not in ('Marketing', 'Marketing Team') or old.employee_id <> public.current_employee_id() then
  ...
  if new.employee_id <> old.employee_id or new.id <> old.id or new.created_at <> old.created_at
  ...
  if old.status = 'In Progress' and new.status = 'Completed' then
    if new.completed_at is null or nullif(btrim(new.outcome), '') is null
...
create policy "follow_ups_insert" on public.follow_ups for insert to authenticated
  with check (
    public.is_admin()
    or (public.current_role_name() in ('Marketing', 'Marketing Team') and employee_id = public.current_employee_id())
  );
