...
export const DirectorDashboard = () => {
  ...
  const cards = [
    ...
    ['Plans Awaiting Approval', pendingBatches.length, FileText, '/director/tour-plans'],
  ...
  return <div className="ds-page director-dashboard-final">
    ...
    <SectionCard title="Pending Director Actions" description="Items that need your attention" actions={<Button variant="secondary" onClick={() => navigate('/director/tour-plans')}>View All</Button>}><DataTable rows={pendingActions} columns={[{ key: 'type', label: 'Type' }, { key: 'record', label: 'Plan / Report / Follow-up / Tender' }, { key: 'status', label: 'Status', render: (row) => <Badge tone="warning">{row.status}</Badge> }, { key: 'action', label: 'Action', render: (row) => <Button variant="secondary" onClick={() => navigate(row.path)}>Review</Button> }]} empty={<EmptyState icon={CheckCircle2} title="No pending Director actions" description="New approvals and exceptions will appear here." />} /></SectionCard>

    <SectionCard title="Today Team Schedule" actions={<Button variant="secondary" onClick={() => navigate('/director/today-schedule')}>View All</Button>}><DataTable rows={visitPlans.filter((plan) => plan.visitDate === todayValue).slice(0, 5)} columns={[{ key: 'employee', label: 'Employee', render: (row) => nameFor(row.employeeId, row.fullName) }, { key: 'time', label: 'Time', render: (row) => row.expectedTime || 'Not provided' }, { key: 'area', label: 'Area', render: (row) => row.area || row.city || row.district || 'Not provided' }, { key: 'customer', label: 'Customer', render: (row) => row.customerName || 'Not provided' }, { key: 'status', label: 'Status', render: (row) => <Badge>{normalizePlanStatus(row.status)}</Badge> }]} empty={<EmptyState icon={Calendar} title="No visits scheduled today" />} /></SectionCard>
