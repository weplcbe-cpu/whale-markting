...
export const FollowUpManagement = () => {
  ...
  const openAdd = () => { setFormData(blankForm()); setEditor({ mode: 'add' }); };
  const openEdit = (row) => { setFormData({ customerId: row.customerId || '', customerName: row.customerName || '', followUpDate: row.followUpDate || '', type: row.type || 'Phone Call', purpose: row.purpose || '', priority: row.priority || 'Medium', notes: row.notes || '' }); setEditor({ mode: 'edit', row }); };
  ...
  return <div className="ds-page follow-up-page"><PageHeader title="Follow-ups" description="Keep every field commitment visible and on schedule." actions={<Button onClick={openAdd}><Plus size={16} /> Add Follow-up</Button>} />
    ...
    <Modal open={Boolean(editor)} onClose={() => setEditor(null)} dirty={Boolean(editor) && Object.values(formData).some(Boolean)} title={editor?.mode === 'edit' ? 'Edit Follow-up' : 'Schedule New Follow-up'} footer={<><Button variant="secondary" onClick={() => setEditor(null)}>Cancel</Button><Button loading={busy} type="submit" form="follow-up-form">{editor?.mode === 'edit' ? 'Save Changes' : 'Schedule Follow-up'}</Button></>}>
    ...
    <Modal open={action?.type === 'start'} onClose={() => setAction(null)} title="Start Follow-up?" size="sm" footer={<><Button variant="secondary" onClick={() => setAction(null)}>Cancel</Button><Button loading={busy} onClick={() => run(() => startFollowUp(action.row.id))}>Start Follow-up</Button></>}><dl className="ds-summary"><div><dt>Organization</dt><dd>{action?.row.customerName || 'Not provided'}</dd></div><div><dt>Due Date</dt><dd>{dateLabel(action?.row.followUpDate)}</dd></div></dl></Modal>
    <Modal open={action?.type === 'delete'} onClose={() => setAction(null)} title="Delete Follow-up?" size="sm" footer={<><Button variant="secondary" onClick={() => setAction(null)}>Cancel</Button><Button variant="danger" loading={busy} onClick={() => run(() => deleteFollowUp(action.row.id))}>Delete Follow-up</Button></>}><dl className="ds-summary"><div><dt>Organization</dt><dd>{action?.row.customerName || 'Not provided'}</dd></div><div><dt>Due Date</dt><dd>{dateLabel(action?.row.followUpDate)}</dd></div></dl></Modal>
    ...
    <Modal open={action?.type === 'details'} onClose={() => setAction(null)} title="Follow-up Details" footer={<Button variant="secondary" onClick={() => setAction(null)}>Close</Button>}><dl className="ds-summary">{action?.row && [['Organization / Person', action.row.customerName], ['Created Date', action.row.createdAt ? new Date(action.row.createdAt).toLocaleString() : 'Not provided'], ['Due Date', dateLabel(action.row.followUpDate)], ['Type', action.row.type], ['Priority', action.row.priority], ['Status', action.row.status], ['Purpose / Objective', action.row.purpose], ['Internal Notes', action.row.notes], ['Started At', action.row.startedAt ? new Date(action.row.startedAt).toLocaleString() : null], ['Completed At', action.row.completedAt ? new Date(action.row.completedAt).toLocaleString() : null], ['Outcome / Result', action.row.outcome], ['Completion Notes', action.row.completionNotes], ['Previous Date', action.row.previousFollowUpDate ? dateLabel(action.row.previousFollowUpDate) : null], ['Reschedule Reason', action.row.rescheduleReason]].filter(([, value]) => value).map(([label, value]) => <div key={label}><dt>{label}</dt><dd>{value}</dd></div>)}</dl></Modal>
