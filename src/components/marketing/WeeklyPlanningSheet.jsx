...
export const WeeklyPlanningSheet = () => {
  ...
  return <div className="ds-page ds-weekly-plan"><PageHeader title="Weekly Visit Planning" description="Create and review the upcoming week’s field visit schedule." actions={<Badge tone={status === 'Draft' ? 'neutral' : 'success'}>{status}</Badge>} />
    ...
    <Modal open={reviewing} onClose={() => setReviewing(false)} title="Review Weekly Plan" subtitle={`${weekFrom} to ${weekTo}`} footer={<><Button variant="secondary" onClick={() => setReviewing(false)}>Back to Edit</Button><Button onClick={submit} disabled={invalidRows.length > 0}><Send size={16} /> Submit Visit Plan</Button></>}><div className="ds-week-review">{invalidRows.length > 0 && <div className="ds-error" role="alert">{invalidRows.length} visit {invalidRows.length === 1 ? 'has' : 'have'} missing required information. Return to edit the highlighted cards.</div>}<div className="ds-week-review__head"><strong>Date</strong><strong>Area</strong><strong>Purpose</strong><strong>Products</strong><strong>Requirement</strong><strong>Priority</strong></div>{rows.map((row) => <div className={hasErrors(row) ? 'has-errors' : ''} key={row.id}><span>{displayDate(row.visitDate)}</span><span>{row.area || 'Missing'}</span><span>{row.visitPurpose || 'Missing'}</span><span>{row.products.join(', ') || 'Optional / none'}</span><span>{row.requirement || 'Missing'}</span><Badge tone={row.priority === 'High' ? 'danger' : 'neutral'}>{row.priority}</Badge></div>)}</div></Modal>
